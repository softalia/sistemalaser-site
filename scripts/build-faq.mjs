...
function readFaq(root) {
  ...
  if (!Array.isArray(data.sections)) throw new Error("faq/faq.json must include a sections array");

  const ids = new Set();
  for (const section of data.sections) {
    if (!section.id || !section.name || !Array.isArray(section.pages) || !Array.isArray(section.questions)) {
      throw new Error("Every FAQ section needs id, name, pages and questions");
    }
    for (const item of section.questions) {
      if (!item.id || !item.question || !item.answer) throw new Error(`Invalid FAQ item in ${section.id}`);
...
function renderAccordion(items, accordionId) {
  return `<div class="accordion accordion-flush" id="${escapeHtml(accordionId)}">${items.map((item, index) => {
...
function removeFaqSchemas(html) {
  return html.replace(/<script\s+type="application\/ld\+json">([\s\S]*?)<\/script>/g, (full, raw) => {
    let data;
    try { data = JSON.parse(raw); } catch { return full; }
    if (Array.isArray(data["@graph"])) {
      const graph = data["@graph"].filter((entry) => entry["@type"] !== "FAQPage");
...
function faqPageHtml(data) {
  const all = data.sections.flatMap((section) => section.questions.map((item) => ({ ...item, section: section.id, sectionName: section.name })));
  const schema = {
    "@context": "https://schema.org",
    "@graph": [
      {
        ...
        mainEntity: all.map((item) => ({ "@type": "Question", name: item.question, acceptedAnswer: { "@type": "Answer", text: item.answer } })),
      },
      {
        "@type": "BreadcrumbList",
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Perguntas frequentes", item: `${siteUrl}/faq.html` },
        ],
      },
    ],
  };
...
<head>
  ...
  <link rel="icon" href="assets/img/icons/favicon-32x32.png"><link rel="stylesheet" href="assets/bootstrap/css/bootstrap.min.css"><link rel="stylesheet" href="assets/css/site-2026.css"><script src="assets/js/header-2026.js"></script><script src="assets/js/footer-2026.js"></script><script src="assets/js/theme-2026.js"></script>
...
<body><noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-PPW847MH" height="0" width="0" style="display:none"></iframe></noscript><a class="skip-link" href="#conteudo">Ir para o conteúdo</a>
  ...
  <script>const faqData=${jsonScript(all)};(${function (faq) {
    ...
    function render() {
      ...
      const filtered = faq.filter((item) => (!section.value || item.section === section.value) && (!term || `${item.question} ${item.answer} ${item.sectionName}`.toLocaleLowerCase("pt-BR").includes(term)));
      status.textContent = `${filtered.length} ${filtered.length === 1 ? "resposta encontrada" : "respostas encontradas"}.`;
      results.innerHTML = "";
      const groups = new Map();
      filtered.forEach((item) => { if (!groups.has(item.section)) groups.set(item.section, []); groups.get(item.section).push(item); });
      groups.forEach((items, sectionId) => {
        const wrapper = document.createElement("section"); wrapper.className = "mb-5"; wrapper.id = sectionId;
        const title = document.createElement("h2"); title.className = "section-title mb-4"; title.textContent = items[0].sectionName; wrapper.appendChild(title);
        const accordion = document.createElement("div"); accordion.className = "accordion accordion-flush"; accordion.id = `faq-page-${sectionId}`;
        items.forEach((item, index) => { const itemId = `faq-page-${item.id}`; const entry = document.createElement("div"); entry.className = "accordion-item"; entry.innerHTML = `<h3 class="accordion-header"><button class="accordion-button${index ? " collapsed" : ""}" type="button" data-bs-toggle="collapse" data-bs-target="#${itemId}">${item.question}</button></h3><div id="${itemId}" class="accordion-collapse collapse${index ? "" : " show"}" data-bs-parent="#${accordion.id}"><div class="accordion-body"></div></div>`; entry.querySelector(".accordion-body").textContent = item.answer; accordion.appendChild(entry); });
        wrapper.appendChild(accordion); results.appendChild(wrapper);
      });
    }
    search.addEventListener("input", render); section.addEventListener("change", render); render();
...
export function buildFaq(root) {
  ...
  fs.writeFileSync(path.join(faqDir, "faq.json"), `${JSON.stringify(data, null, 2)}\n`);
  fs.writeFileSync(path.join(dist, "faq.html"), faqPageHtml(data));

  const pages = new Map();
  for (const section of data.sections) for (const page of section.pages) pages.set(page, section);
  for (const [page, section] of pages) {
    const file = path.join(dist, page);
    if (!fs.existsSync(file)) throw new Error(`FAQ page target does not exist: ${page}`);
    let html = fs.readFileSync(file, "utf8");
    html = removeVisibleFaq(removeFaqSchemas(html));
    html = html.replace("</main>", `${pageFaqSection(section)}\n  </main>`);
    html = html.replace("</head>", `  <script type="application/ld+json" data-generated-faq-schema="${escapeHtml(section.id)}">${jsonScript(faqSchema(section, page))}</script>\n</head>`);
